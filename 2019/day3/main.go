...
func FewestStepsWireIntersection(w1 []string, w2 []string) int {
	p1, err := NewPathFromWire(w1)
	if err != nil {
		panic(err)
	}
...
func main() {
	minDist := NearestWireIntersection(wire1, wire2)
	fmt.Printf("result(min-dist): %d\n", minDist)
	minSteps := FewestStepsWireIntersection(wire1, wire2)
	fmt.Printf("result(min-steps): %d\n", minSteps)
}
